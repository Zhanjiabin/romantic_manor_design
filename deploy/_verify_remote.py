...
cmds = [
    ...
    'bash -lc \'set -a; source /opt/manor-desk/.env; set +a; cookie=$(mktemp); trap "rm -f $cookie" EXIT; echo -n login:; curl -s -o /dev/null -w %{http_code} -c "$cookie" -H "Content-Type: application/x-www-form-urlencoded" --data-urlencode "user=$MANOR_USER" --data-urlencode "password=$MANOR_PASSWORD" --data-urlencode "next=/" http://127.0.0.1:8095/api/login; echo; echo -n papers:; curl -s -o /dev/null -w %{http_code} -b "$cookie" http://127.0.0.1:8095/api/saves/building/papers; echo; echo -n html:; curl -s -o /dev/null -w %{http_code} -b "$cookie" http://127.0.0.1:8095/web/building.html; echo; echo -n cloth:; curl -s -o /dev/null -w %{http_code} -b "$cookie" http://127.0.0.1:8095/web/cloth.html; echo; echo -n catalog:; curl -s -o /dev/null -w %{http_code} -b "$cookie" http://127.0.0.1:8095/data/cloth_catalog.json; echo; echo -n jsver:; curl -s -b "$cookie" http://127.0.0.1:8095/web/cloth.html | grep -o "cloth.js?v=[0-9]*"\'',
]
for cmd in cmds:
