...
- 第 6–8 个字符是素材 ID。单包图纸是当前 `mat.cfg` 的 1xx–6xx；混包图纸是
  ...
  `exhibit.tab` 行号代替；两者会命中“存在但完全不相干”的 ALE。
- **已锁定（2026-08-30，用户对照游戏截图确认）**：只使用当前
  `data/building_pack_uids.json`。关键对照：`14124`→bazaar，`15512`→supermarket，
  `19160`→giant，`20129`→japan，`21161`→tds。不得改现有 UID，不得用 `框架` 借图。
- `mat.cfg` 的 `框架=(...)` 是建筑框架兼容列表，不是素材包 UID，也不是跨包借图表。
  缺号时必须保持缺失，不能从所谓“框架包”随便找同号素材。
