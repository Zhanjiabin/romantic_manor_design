/* Isolated remodel desk. Generated from a building-desk snapshot; do not edit building.js. */
const DESIGN_W = 570;
const DESIGN_H = 550;
...
const CUSTOMS_KEY = "manor-remodel-customs-v1";
const SESSION_KEY = "manor-remodel-session-v1";
const ASSET_PREFS_KEY = "manor-remodel-asset-prefs-v1";
const HUD_LAYOUT_KEY = "manor-remodel-hud-layout-v2";
const SHEET_LAYOUT_KEY = "manor-remodel-sheet-layout-v1";
const MATERIALS_DOCK_KEY = "manor-remodel-materials-dock-collapsed";
const SMART_STYLES_KEY = "manor-remodel-smart-styles-v1";
...
async function bootRemodel() {
  ...
  const [catalog, uidCatalog, packUids, itemIcons, semanticStyles] = await Promise.all([
    ...
    fetch("/data/building_uid_map.json")
      ...
      .catch(() => ({ packs: [] })),
    fetch("/data/item_pack_uids.json")
  ...
  warmOtherDesk("/web/cloth.html", ["/web/cloth.js?v=52", "/data/cloth_catalog.json"]);
}
...
async function beginDesign() {
  ...
  applyRemodelKindRules(state.base);
  if (!state.base) {
    await appAlert("请先选择基座。", { title: "还没选基座" });
    return;
  }
  const count = placedDesignCount();
  if (count > 0) {
...
function updateBase() {
  ...
  if (hint) {
    ...
    hint.textContent = state.paperBaseHint ? `图纸原基座 ${state.paperBaseHint}` : "";
  }
...
async function fetchBuildingSaves() {
  try {
    const res = await fetch("/api/saves/remodel", { credentials: "same-origin" });
...
function putBuildingSaves(payload) {
  return fetch("/api/saves/remodel", {
    method: "PUT",
...
function buildingSessionSnapshot() {
  return {
    ...
    scaleMode: Number(state.scaleMode) || 0,
...
function restoreBuildingSession(remoteSnap) {
  ...
  try {
    ...
    if (snap.showMan != null) {
      state.showMan = !!snap.showMan;
      state._lastShowManKind = Number(state.base?.kind) || 0;
    }
    if (snap.scaleMode != null) state.scaleMode = Number(snap.scaleMode) || 0;
    if (typeof snap.userAction === "string") state.userAction = snap.userAction;
...
const NATIVE_CLIP_MAX = 0x4fff; // rc3.exe GetClipboard 0x669740

function encodeNativeDesignClip(records) {
  // TxtExport 0x6689b0: "V1" + ";" + 9-char desk records (xy 5 / mat 3 / state 1).
  const min = -0x4000;
  const max = 0x3fff;
  const rows = (Array.isArray(records) ? records : [])
    ...
    .map((row) => ({
      mode: "desk",
      x: Math.max(min, Math.min(max, Math.round(Number(row.x) || 0))),
      y: Math.max(min, Math.min(max, Math.round(Number(row.y) || 0))),
      mat: Math.max(0, Math.round(Number(row.mat) || 0)),
      state: Math.max(0, Math.min(63, Math.round(Number(row.state ?? row.flip) || 0))),
    }));
  if (!rows.length) return "";
  const text = formatV1(rows, "desk");
  if (/[\u0000-\u001f]/.test(text)) {
    throw new Error("native GetClipboard stops at control characters");
  }
  return text;
}
...
function nativeCopyIndices() {
  const selected = selectionInLayerOrder(state.selected);
  if (selected.length) return selected;
  return state.records
    .map((_, index) => index)
    .filter((index) => state.records[index] && !state.records[index].hidden);
}
...
function showNativeClipFallback(text) {
  const field = document.getElementById("remodelClipText");
  const hint = document.getElementById("remodelClipHint");
  if (field) {
    field.value = text;
    field.focus();
    field.select();
  ...
  if (hint) {
    hint.textContent = "浏览器没把内容写进系统剪贴板。点下面的框，按 Ctrl+C（手机长按复制），再到游戏物件设计桌 Ctrl+V。";
  }
  setModalVisible("dlgRemodelClip", true);
}

function teachGamePaste(count) {
...
async function copyRecordsToGame(indices, options = {}) {
  ...
  let text;
  try {
    text = encodeNativeDesignClip(records);
  } catch (error) {
    if (typeof appAlert === "function") {
      await appAlert(error.message || String(error), { title: "无法拷贝" });
    }
    return false;
  }
  if (text.length > NATIVE_CLIP_MAX) {
    if (typeof appAlert === "function") {
      await appAlert("选中的内容超过游戏剪贴板上限。请少选一些再复制。", { title: "无法拷贝" });
    }
    return false;
  }
  const ok = await writeClipText(text);
  if (ok) {
    if (options.teach) teachGamePaste(records.length);
  } else if (options.fallback !== false) {
    showNativeClipFallback(text);
  }
  return ok;
}

async function copyToGame(options = {}) {
  const indices = nativeCopyIndices();
  const selected = selectionInLayerOrder(state.selected);
  if (selected.length) state.clipboard = serializeClipboardRecords(selected);
  return copyRecordsToGame(indices, {
    teach: options.teach !== false,
    fallback: options.fallback !== false,
    alertEmpty: options.alertEmpty !== false,
  });
}
...
function copySelected() {
  ...
  copyRecordsToGame(indices, { teach: false, fallback: false, alertEmpty: false }).catch((error) => {
    console.warn(error);
  });
}
...
const NUDGE_STEP_KEY = "manor-remodel-nudge-step";
...
const COMMANDS = [
  ...
  { id: "copy", label: "复制到剪贴工作集", shortcut: "Ctrl+C", run: copySelected },
  {
    ...
    run: () => copyToGame().catch((error) => console.warn(error)),
  },
...
function openCanvasContextMenu(client, scene) {
  ...
  if (selected.length) {
    ...
    items.push({ label: "复制到游戏", shortcut: "Ctrl+Shift+C", run: () => executeCommand("copyToGame") });
...
async function clearServerPaperLibrary() {
  ...
  if (!ok) return;
  try {
    await fetch("/api/saves/remodel/papers", { method: "DELETE", credentials: "same-origin" });
  } catch (error) {
    console.warn(error);
  }
...
function applyDeskDocumentMeta(doc) {
  ...
  if (doc.showMan != null) {
    state.showMan = !!doc.showMan;
    state._lastShowManKind = Number(state.base?.kind) || 0;
  }
  if (doc.scaleMode != null) state.scaleMode = Number(doc.scaleMode) || 0;
  if (typeof doc.userAction === "string") state.userAction = doc.userAction;
  applyRemodelKindRules(state.base);
...
function bindBuilding() {
  ...
  const onShowMan = (event) => {
    state.showMan = event.target.checked;
    syncShowManChecks();
    markBuildingDirty();
    renderBuilding();
  };
  const showMan = document.getElementById("showMan");
  const showManDesign = document.getElementById("showManDesign");
  if (showMan) showMan.onchange = onShowMan;
  if (showManDesign) showManDesign.onchange = onShowMan;
  ...
  const btnPlaceOnTerrain = document.getElementById("btnPlaceOnTerrain");
  if (btnPlaceOnTerrain) {
    btnPlaceOnTerrain.onclick = () => {
  ...
  ["dlgCommands", "dlgShortcuts", "dlgSaveDesign", "dlgImageBuilding", "dlgRemodelClip"].forEach((id) => {
    document.getElementById(id)?.addEventListener("click", (event) => {
      if (event.target.id === id) setModalVisible(id, false);
    });
  });
  document.getElementById("btnRemodelClipSelect")?.addEventListener("click", () => {
    const field = document.getElementById("remodelClipText");
    if (!field) return;
    field.focus();
    field.select();
    field.setSelectionRange(0, field.value.length);
    writeClipText(field.value).catch((error) => console.warn(error));
  });
  applyRailState();
  ...
  window.addEventListener("keydown", (event) => {
    ...
    if (event.key === "Delete" || event.key === "Backspace") {
      ...
      event.preventDefault();
      executeCommand(event.shiftKey ? "copyToGame" : "copy");
...
bootRemodel().catch((error) => {
  console.error(error);
  appAlert("改造设计桌启动失败：" + (error.message || error), { title: "启动失败" });
});
